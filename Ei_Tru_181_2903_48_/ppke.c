...
int ppke_keypair(unsigned char *pk, unsigned char* sk,unsigned char arr_seed[N3_SAMPLE_FG_BYTES]){
    ...
    while(flag==0){
        ...
        // }
        // printf("\n**************************\n");
        double execution_time_inverse = 0;
        double start = cpucycles_start();
        flag = Almost_inverse_ZpwCnC3(f,Q,fq);
        // poly_Zw_print(fq,N3);
        double end = cpucycles_stop();

        execution_time_inverse = (double)(end - start);
        execution_time_inverse = ((double)(execution_time_inverse)*1000)/CPUCYCLES;

        printf("Time: %f\n",execution_time_inverse);
