...
#define CPUCYCLES (3*1000000000.0);

int main(){
    printf("Inside the main functio\n");

    char fn_req[32], fn_rsp[32];
    FILE *fp_req, *fp_rsp;
    unsigned char seed[48];
    unsigned char message_seed[N3_SAMPLE_M_BYTES];
    unsigned char input_message[PPKE_MESSAGEBYTES];
    unsigned char entropy_input[48];
    unsigned char ct[PPKE_CIPHERTEXTBYTES];
    int count;
    int done;
    unsigned char pk[PPKE_CIPHERTEXTBYTES];
    unsigned char sk[PPKE_SECRETKEYBYTES];
    int ret_val;
    ...
    sprintf(fn_req, "PQCccaKAT_%d.req",CRYPTO_SECRETKEYBYTES);//
    if((fp_req = fopen(fn_req, "w")) == NULL ){
        printf("Couldn't open <%s> for write\n",fn_req);
        return KAT_FILE_OPEN_ERROR;
    }

    sprintf(fn_rsp, "PQCccaKAT_%d.rsp", CRYPTO_SECRETKEYBYTES);
    ...
    for(int i = 0;i<trials;i++){
        ...
        randombytes(seed,48);
        randombytes(message_seed,N3_SAMPLE_M_BYTES);

        Term message[N3];

        sample_lm(message, message_seed);

        poly_S3m_tobytes(input_message,message);
   ...
   // printf("clock per second: %ld \n", CLOCKS_PER_SEC);
    ...
    printf("Average Time taken to execute in milliseconds : %f (ms) \n", execution_time);
